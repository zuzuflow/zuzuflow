import type { WorkflowNodeProps } from "../../types/nodeProps";
import React from "react";
import type { GcpBigQueryConfig } from "@workflow/shared";
import { NodeWrapper } from "../canvas/NodeWrapper";

export function GcpBigQueryNode({ id, data, selected }: WorkflowNodeProps): React.ReactElement {
  const cfg = data.config as GcpBigQueryConfig;
  const target = cfg.operation === "insertRows" ? `${cfg.datasetId ?? ""}.${cfg.tableId ?? ""}` : "";
  return (
    <NodeWrapper nodeId={id} kind={data.kind} label={data.label} selected={selected} style={data.style}>
      <div className="text-[10px] text-slate-300">
        <span className="font-semibold">{cfg.operation ?? "query"}</span>
        {target && <span className="text-slate-500"> · {target}</span>}
      </div>
    </NodeWrapper>
  );
}
