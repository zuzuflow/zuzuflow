...
import type { GitConfig, GitProvider, GitStatus, UserPublic, ApiTokenPublic, ApiTokenCreated, EnvironmentItem, EnvironmentMember, OrgDetail } from "../lib/api";
import {
  getGitConfig, saveGitConfig, getGitStatus, gitPush, gitPull,
  listUsers, createUser, changePassword, deleteUser,
  listApiTokens, createApiToken, revokeApiToken,
  listEnvironments, createEnvironment, updateEnvironment, deleteEnvironment,
  listEnvironmentMembers, addEnvironmentMember, removeEnvironmentMember,
...
function ResultPill({ ok, msg }: { ok: boolean; msg: string }) {
  return (
    <span className={cn("flex items-center gap-1 text-xs", ok ? "text-emerald-400" : "text-red-400")}>
...
function GitTab() {
  ...
  const loadStatus = useCallback(async () => {
    ...
    try { setGitStatus(await getGitStatus()); } catch { /* best-effort */ }
  ...
  async function handlePush() {
    ...
    try { const r = await gitPush(); setPushResult({ ok: true, msg: r.message }); loadStatus(); }
    catch (err) { setPushResult({ ok: false, msg: (err as Error).message }); }
    finally { setPushing(false); }
  }

  async function handlePull() {
    setPulling(true); setPullResult(null);
    try { const r = await gitPull(); setPullResult({ ok: true, msg: r.message }); loadStatus(); }
    catch (err) { setPullResult({ ok: false, msg: (err as Error).message }); }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Link a remote repository to push and pull workflows as JSON files. Tokens are stored encrypted.
        </p>
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 space-y-4">
            ...
            <div>
              ...
              <input className={inputClass} placeholder="https://github.com/org/repo.git"
                value={repoUrl} onChange={(e) => setRepoUrl(e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>Branch</label>
              <input className={inputClass} placeholder="main"
                value={branch} onChange={(e) => setBranch(e.target.value)} />
            </div>
            {(provider === "bitbucket" || provider === "custom") && (
              <div>
                <label className={labelClass}>Username</label>
                <input className={inputClass} placeholder="your-username"
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={tokenMasked ? "Leave blank to keep existing token" : "ghp_…"}
                  value={token}
              ...
              {tokenMasked && <p className="text-[10px] text-muted-foreground mt-0.5">Token is stored — edit to replace it.</p>}
            </div>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          <button onClick={loadStatus} disabled={statusLoading}
            ...
            <RefreshCw size={11} className={cn(statusLoading && "animate-spin")} />
            Refresh
          </button>
        </div>
        <p className="text-xs text-muted-foreground mb-4">
          Push exports all workflows to the repo as JSON. Pull imports them back and upserts into the database.
        </p>
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          {gitStatus && (
            <div className="px-5 py-3 border-b border-border grid grid-cols-3 gap-4 text-xs">
              <div>
                ...
                <span className={cn("font-medium", gitStatus.configured ? "text-emerald-400" : "text-amber-400")}>
                  {gitStatus.configured ? "Connected" : "Not configured"}
                </span>
              </div>
              <div>
                <p className="text-muted-foreground mb-0.5">Last push</p>
                <p className="text-foreground font-mono">{fmtDate(gitStatus.lastPush)}</p>
              </div>
              <div>
                <p className="text-muted-foreground mb-0.5">Last pull</p>
                <p className="text-foreground font-mono">{fmtDate(gitStatus.lastPull)}</p>
              </div>
            </div>
          )}
          {/* Auto-sync toggles */}
          ...
          <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Export all workflows to the remote repository, organized by folder.</p>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              {pushResult && <ResultPill {...pushResult} />}
              <button onClick={handlePush} disabled={pushing || !gitStatus?.configured}
          ...
          <div className="px-5 py-4 flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-medium text-foreground mb-0.5">Pull from Git</p>
              <p className="text-xs text-muted-foreground">Import workflows from the repository and upsert them into the database.</p>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              {pullResult && <ResultPill {...pullResult} />}
              <button onClick={handlePull} disabled={pulling || !gitStatus?.configured}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded bg-muted hover:bg-slate-600 text-foreground transition-colors disabled:opacity-40">
...
function UsersTab() {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    if (!newUsername.trim() || !newEmail.trim() || !newPassword.trim()) { setCreateError("Username, email, and password are required"); return; }
    setCreating(true); setCreateError("");
    try {
      await createUser(newUsername.trim(), newEmail.trim(), newPassword.trim(), newRole);
  ...
  async function handleChangePw(e: React.FormEvent) {
    ...
    } catch (err) { setChangePwError((err as Error).message); }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Manage who can access this application. Each user has a role: admin or editor.
        </p>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          {loading ? (
            ...
            <div className="px-5 py-4 text-xs text-muted-foreground">No users found.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {users.map((u) => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell className="text-muted-foreground font-mono text-xs">{u.lastLoginAt ? fmtDate(u.lastLoginAt) : "Never"}</TableCell>
                    <TableCell className="text-muted-foreground font-mono text-xs">{u.lastLoginIp || "—"}</TableCell>
                    <TableCell className="text-muted-foreground font-mono">{fmtDate(u.createdAt)}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => { setChangePwUser(u); setNewPw(""); setChangePwError(""); }}
      ...
      {/* Change password modal */}
      <Dialog open={!!changePwUser} onOpenChange={(open) => { if (!open) { setChangePwUser(null); setNewPw(""); } }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Change password</DialogTitle>
            <DialogDescription>For user: <span className="text-foreground">{changePwUser?.username}</span></DialogDescription>
          </DialogHeader>
          <form onSubmit={handleChangePw} className="space-y-3">
            ...
            {changePwError && <p className="text-xs text-red-400">{changePwError}</p>}
          </form>
          <DialogFooter>
            <Button variant="ghost" onClick={() => { setChangePwUser(null); setNewPw(""); }}>Cancel</Button>
            <Button onClick={handleChangePw as any} disabled={changingPw}>
              {changingPw ? <Loader2 size={12} className="animate-spin mr-1.5" /> : <Check size={12} className="mr-1.5" />}
...
function ApiTokensTab() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setTokens(await listApiTokens()); setError(""); }
    catch (err) { setError((err as Error).message); }
    finally { setLoading(false); }
  }, []);
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    if (!newName.trim()) { setCreateError("Token name is required"); return; }
    setCreating(true); setCreateError("");
    try {
  ...
  async function handleConfirmRevoke() {
    ...
    const target = deleteTarget;
    setDeleteTarget(null);
    try { await revokeApiToken(target.id); await load(); }
    catch (err) { toast.error((err as Error).message); }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Generate long-lived API tokens for external integrations (e.g., npm packages, CI pipelines). Tokens are shown once — store them securely.
        </p>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          {loading ? (
            ...
            <div className="px-5 py-4 text-xs text-muted-foreground">No tokens yet. Generate one below.</div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {tokens.map((t) => (
                  <TableRow key={t.id}>
                    ...
                    <TableCell className="text-muted-foreground font-mono">{fmtDate(t.createdAt)}</TableCell>
                    <TableCell className="text-muted-foreground font-mono">{fmtDate(t.lastUsedAt)}</TableCell>
                    <TableCell className="text-right">
                      <button onClick={() => handleRevoke(t.id, t.name)}
                        className="text-muted-foreground hover:text-red-400 transition-colors" title="Revoke token">
                        <Trash2 size={12} />
                      </button>
                    </TableCell>
                  </TableRow>
      ...
      {revealed && (
        <div className="bg-emerald-950/40 border border-emerald-700/40 rounded-xl px-5 py-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs font-semibold text-emerald-400">Token generated — copy it now</p>
          </div>
          <p className="text-xs text-muted-foreground mb-3">
            This is the only time the full token will be shown. It will not be visible again after you leave this page.
          </p>
          <div className="flex items-center gap-2">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <Plus size={14} className="text-muted-foreground" />
          <h3 className="text-sm font-semibold text-foreground">Generate Token</h3>
        </div>
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <form onSubmit={handleCreate} className="px-5 py-4 space-y-3">
            <div>
              <label className={labelClass}>Token name</label>
              <input className={inputClass} type="text" placeholder="e.g. My App Integration"
...
function EnvironmentsTab() {
  ...
  const loadMembers = useCallback(async (env: EnvironmentItem) => {
    setSelectedEnv(env);
    try {
      const [m, u] = await Promise.all([listEnvironmentMembers(env.id), listUsers()]);
      setMembers(m);
      setAllUsers(u);
    } catch { /* ignore */ }
  }, []);
  ...
  const envColors: Record<string, string> = {
    production: "text-green-400", staging: "text-yellow-400", development: "text-blue-400", dev: "text-blue-400", test: "text-purple-400",
  };
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">
          Environments isolate workflows, credentials, and variables. Each environment has its own members.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {envs.map((env) => (
            <div key={env.id} className={cn(
              ...
              selectedEnv?.id === env.id ? "border-indigo-500 bg-indigo-500/5" : "border-border"
            )} onClick={() => loadMembers(env)}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Globe size={14} className={envColors[env.slug] ?? "text-indigo-400"} />
                  <span className="text-sm font-medium">{env.name}</span>
                  <span className="text-[10px] font-mono text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{env.slug}</span>
                  {env.isDefault && <span className="text-[10px] bg-green-500/10 text-green-400 px-1.5 py-0.5 rounded">Default</span>}
                </div>
                {!env.isDefault && (
                  <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(env); }} className="text-muted-foreground hover:text-red-400">
                    <Trash2 size={12} />
                  </button>
                )}
              </div>
              <p className="text-[10px] text-muted-foreground mt-1">Created {fmtDate(env.createdAt)}</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="border border-border rounded-lg p-4 mt-4">
          ...
          <Table>
            ...
            <TableBody>
              {members.map((m) => (
                <TableRow key={m.id}>
                  <TableCell className="text-xs font-mono">{m.username}</TableCell>
                  <TableCell>
                    <span className={cn("text-xs px-1.5 py-0.5 rounded", m.role === "admin" ? "bg-indigo-500/10 text-indigo-400" : "bg-muted text-muted-foreground")}>
                      {m.role}
                    </span>
                  </TableCell>
                  <TableCell>
                    <button onClick={() => handleRemoveMember(m.id)} className="text-muted-foreground hover:text-red-400">
                      <Trash2 size={11} />
                    </button>
                  </TableCell>
                </TableRow>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Create a new isolated environment for workflows, credentials, and variables.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div>
              <label className={labelClass}>Name</label>
              <input className={inputClass} value={newName} onChange={(e) => {
            ...
            <div>
              ...
              <input className={cn(inputClass, "font-mono")} value={newSlug} onChange={(e) => setNewSlug(e.target.value)} placeholder="e.g., staging" />
              <p className="text-[10px] text-muted-foreground mt-0.5">Used in API URLs. Lowercase letters, numbers, and hyphens only.</p>
            </div>
          </div>
          <DialogFooter>
...
function OrganizationTab() {
  ...
  async function handleSave() {
    setSaving(true); setSaveResult(null);
    try {
      const updated = await updateOrganization({ name: name.trim(), address: address.trim() });
      setOrg(updated);
  ...
  if (!org) {
    return <div className="px-5 py-4 text-xs text-muted-foreground">No organization context. Please select an organization first.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Manage your organization's name and address. Only admins and owners can edit these settings.
        </p>
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 space-y-4">
            ...
            <div>
              <label className={labelClass}>Slug</label>
              <input
                className={inputClass + " font-mono bg-muted/50 cursor-not-allowed"}
                type="text"
                value={org.slug}
                disabled
              />
